...
audpc <- function(fieldbook, type = "absolute", use_stairs = FALSE,
  ...
  #row.names(fb) = fieldbook$INSTN %>% as.character
  #fb <- cbind(fieldbook$INSTN, fb)
  # get dates from metadata
  cm = attr(fieldbook, "CropManagement")
  if(is.null(cm)) stop("Not a fieldbook with meta data for crop management.")
  dates = cm[stringr::str_detect(cm$`Intervention type`, var_name), "Date"]
  # check if same number as observations
  if(nrow(dates) != ncol(fb)) stop("Number of dates different from number of observations.")
  ...
  for(i in 2:nrow(dates)){
    ...
    days[i] = as.integer(date_now - date_before) + days[i - 1]
  }
  # calculate according to type
  if(!use_stairs){
    xAUDPC <- switch(type,
                  absolute = agricolae::audpc(fb[, -ncol(fb)], days, "absolute"),
                  relative = agricolae::audpc(fb[, -ncol(fb)], days, "relative"),
                  standardized = agricolae::audpc(fb[, -ncol(fb)], days, "absolute")
                  )
  } else {
    xAUDPC <- switch(type,
                     absolute = agricolae::audps(fb[, -ncol(fb)], days, "absolute"),
                     relative = agricolae::audps(fb[, -ncol(fb)], days, "relative"),
                     standardized = agricolae::audps(fb[, -ncol(fb)], days, "absolute")
    )
  ...
  if(variable_only) return(xAUDPC)
  ...
  if(lbl %in% names(fieldbook)){
    ...
    # rescue meta data before changing table structure; otherwise they are lost
...
# fb <- audpc(DF, variable_only = FALSE)
# fb <- audpc(fb, type = "relative", use_stairs = T, variable_only = FALSE)
# fb <- audpc(fb, type = "relative", variable_only = FALSE)
# fb <- audpc(fb, type = "standardized", use_stairs = T, variable_only = FALSE)
